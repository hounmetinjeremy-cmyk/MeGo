// Interfaces
import { ILazyQueryResult } from "@/lib/store/utils/interfaces";
import {
  IStoreByIdResponse,
...
} from "@/lib/store/utils/interfaces/rider.interface";

// Components
import {
  CustomContinueButton,
  NoRecordFound,
} from "@/lib/store/ui/useable-components";
import WithdrawModal from "../form";
import RecentTransaction from "../recent-transactions";

// Hooks
import { useUserContext } from "@/lib/store/context/global/user.context";
...
// GraphQL
import { CREATE_WITHDRAW_REQUEST } from "@/lib/store/apollo/mutations/withdraw-request.mutation";
import {
  STORE_BY_ID,
...
} from "@/lib/store/apollo/queries/store.query";
...
import { WalletScreenMainLoading } from "@/lib/store/ui/skeletons";
  ...
  const [createWithDrawRequest, { loading: createWithDrawRequestLoading }] =
    useMutation(CREATE_WITHDRAW_REQUEST, {
      onCompleted: () => {
        ...
        router.push({
          pathname: "/store/(tabs)/wallet/(routes)/success",
        });
      },
      onError: (error) => {
        setIsBottomModalOpen(false);
