// Interfaces
import { ILazyQueryResult } from "@/lib/rider/utils/interfaces";
import {
  IRiderByIdResponse,
  IRiderCurrentWithdrawRequestResponse,
  // IRiderEarningsResponse,
  IRiderTransactionHistoryResponse,
} from "@/lib/rider/utils/interfaces/rider.interface";
...
import {
  ...
  NoRecordFound,
} from "@/lib/rider/ui/useable-components";
...
import { useLazyQueryQL } from "@/lib/rider/hooks/useLazyQueryQL";
import { useMutation } from "@apollo/client";
import { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";

// GraphQL
import { CREATE_WITHDRAW_REQUEST } from "@/lib/rider/apollo/mutations/withdraw-request.mutation";
import {
  RIDER_BY_ID,
...
import { WalletScreenMainLoading } from "@/lib/rider/ui/skeletons";
  ...
  const [createWithDrawRequest, { loading: createWithDrawRequestLoading }] =
    useMutation(CREATE_WITHDRAW_REQUEST, {
      onCompleted: () => {
        ...
        router.push({
          pathname: "/rider/(tabs)/wallet/(routes)/success",
        });
      },
      onError: (error) => {
        Keyboard.dismiss();
